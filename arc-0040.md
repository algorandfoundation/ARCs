...
This ARC introduces a standardized schema and decentralized (platform-agnostic) workflows for defining, configuring, and validating metadata for sets of associated assets ("Asset Sets"), such as NFT Collections. The proposal provides a modular framework enabling bidirectional linkage between a parent Asset Set ASA, Child Assets, and Contributors (e.g. Creators), for trustless proof of control and attribution. Asset Set ASAs provide a verifiable source of truth for any Asset Set.
...
- Unverified asset Groups
- Other emerging use cases

### 1: Standardized Asset Set Metadata Schema

#### Key Terminology
- **Asset Set** : A group of associated entities such as: NFTs, addresses, other Asset Sets  
...
| `set_links `          | array     | External URLs related to the Asset Set        | No      |
...
3. Mint the Child Assets, encoding the ID of the Asset Set ASA in each Child Asset's metadata.
...
1. Create a placeholder metadata JSON file and upload to IPFS.
2. Mint a mutable ARC-19 asset representing the Asset Set (the "Asset Set ASA"), storing the IPFS hash (from step 1) in the Reserve Address field.
3. Mint any Contributor Assets, encoding the ID of the Asset Set ASA in each Contributor Asset's metadata.
...
2. Mint a mutable ARC-19 asset representing the Asset Set (the "Asset Set ASA"), storing the IPFS hash (from step 1) in the Reserve Address field.
3. All Contributor Addresses must opt-in to the Asset Set ASA to confirm their association.
...
- NFT Collections: Sets of assets with verified association
...
The modular framework maximizes the scope and flexibility of the ARC, but also introduces increased complexity. Alternatively, validation can be limited to a subset of conditions, for example, those requiring bidirectional linkage – this would increase simplicity and robustness.
...
- Incorporates bidirectional linkage which aligns with Child Asset validation.
...
- Contributors can display owned Contributor Assets within a public gallery for more tangible proof of association.
