import {
  decodeAddress as algosdkDecodeAddress,
  encodeUint64 as algosdkEncodeUint64,
} from "algosdk";
...
export type OfflinePartKeyIntegrityHashArgs = Pick<
  PartKeyIntegrityHashArgs,
  "genesisHashB64" | "account"
>;

export function getPartKeyIntegrityHash(
  args: PartKeyIntegrityHashArgs
): string {
  const {
    genesisHashB64,
    account,
  ...
  const rawGenesisHash = decodeBase64(genesisHashB64, 32, "genesisHashB64");
  ...
  const rawStateProofKey = decodeBase64(
    stateProofKeyB64,
    64,
    "stateProofKeyB64"
  );
  ...
  // expected: 32 + 32 + 32 + 32 + 64 + 8 + 8 + 8
  if (raw.length !== 216) {
    throw new Error(
      `Expected concatenated buffer to be 216 bytes, found: ${raw.length}`
    );
  }
...
const emptyBase64length32 = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
const emptyBase64length64 =
  "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA==";
export function getOfflinePartKeyIntegrityHash(
  args: OfflinePartKeyIntegrityHashArgs
): string {
  const { genesisHashB64, account } = args;
  return getPartKeyIntegrityHash({
    genesisHashB64,
    account,
...
const decodeBase64 = (
  ...
  const padding =
    !b64 || b64.length % 4 === 0
      ? ""
      : new Array(4 - (b64?.length % 4)).fill("=").join("");
  ...
  try {
    ...
    throw new Error(
      `Failed to decode field ${fieldName}: ${(e as Error).message}`
    );
  }
  ...
  if (value.length !== expectedLength) {
    throw new Error(
      `Field ${fieldName} was expected to have length ${expectedLength} but found length: ${value.length}`
    );
  }
...
function encodeUint64(n: number, fieldName: string): Uint8Array {
  ...
  if (typeof n !== "number") {
    throw new Error(
      `Field ${fieldName} was expected to be numeric but found: ${typeof n}`
    );
  }
