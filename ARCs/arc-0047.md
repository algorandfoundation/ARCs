...
All ABI types **MUST** be encoded as base16 and prefixed with `0x`, with the exception of `uint64` which should be provided as an integer.

Variable values **MUST** not include any newlines or `;` to prevent code injection from variable values. The exception is `string` variables, which must be verified to ensure all `"` are properly escaped.  
