...
The admin can optionally add a named plugin to their abstracted account application. If the name starts with "ARC" then the name **MUST** match the regex `/^ARC\d+$/`, with the number corresponding to an ARC number. The ARC number **MUST NOT** be prefixed with any 0s. A plugin name beginning with "ARC" must correspond to a plugin that implements the interface(s) standardized in the respective ARC.
