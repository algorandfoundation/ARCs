...
- issuer:
The value of the issuer property MUST be either a URL, or an object containing an id property whose value is a URL; in either case, the issuer selects this URL to identify itself in a globally unambiguous way. It is RECOMMENDED that the URL be one which, if dereferenced, results in a controller document [VC-CONTROLLER-DOCUMENT](https://w3c.github.io/vc-controller-document/) about the issuer that can be used to verify the information expressed in the credential.
