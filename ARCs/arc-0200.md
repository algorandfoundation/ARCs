...
The methods `transfer` and `transferFrom` method MUST error when the balance of `sender` is insufficient.
...
A `Transfer` event SHOULD be emitted, with `sender` being the zero address, when a token is first minted.
A `Transfer` event SHOULD be emitted, with `recipient` being the zero address, when a token is destroyed.
