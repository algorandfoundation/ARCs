...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "readonly": false,
      "args": [
        { "type": "address", "name": "_from", "desc": "The source  of the transfer" },
        { "type": "address", "name": "_to", "desc": "The destination of the transfer" },
        { "type": "uint256", "name": "_value", "desc": "Amount of tokens to transfer"}
      ],
      "returns": { "type": "bool", "desc": "Success" }
    },
    {
...
The methods `transfer` and `transferFrom` method MUST error when the balance of `_from` is insufficient.
...
A `Transfer` event SHOULD be emitted, with `_from` being the zero address, when a token is first minted.
A `Transfer` event SHOULD be emitted, with `_to` being the zero address, when a token is destroyed.
