...
This ARC defines a **standard wallet API for arbitrary data signing**.  
...
- `data`: Base64url encoded `header.payload` of a JWT.
- JWT replay/domain protection must come from claims: `aud`, `iss`, `exp`, `nbf`, `jti`.
- `authenticatorData`: Not used. **MUST** be `null` for this scope.
...
- Wallet **MUST** validate `exp` and `nbf` claims (to reject expired/not-yet-valid tokens).
- Wallet **MUST** reject if both `jti` and `nonce` are absent.
- `iss` and `aud` **MUST** be shown to the user before signing.
...
| `authenticatorData` | `null`   | Not used in JWT scope.                                                                                                                                |
...
| Error                     | Description                                                                |
| ------------------------- | -------------------------------------------------------------------------- |
| `ERROR_INVALID_SCOPE`     | The `scope` is not `2`.                                                    |
...
- `data`: Canonical JSON of OAuth message (e.g., PKCE, DPoP proof).`data` **MUST** include a timestamp field, not just optional, so verifiers can prevent stale proofs.
- Nonce/state/PKCE challenge serve replay protection. Canonical JSON **MUST** be deterministic (sorted keys, UTF-8, no whitespace variance).
- `authenticatorData`: Not used. **MUST** be `null` for this scope.
...
| `requestId`         | `string` | Optional unique request identifier.                                                                                                                        |
| `authenticatorData` | `null`   | Not used in OAUTH scope.                                                                                                                                   |
| `hdPath`            | `string` | Optional BIP44 path for key derivation.                                                                                                                    |
...
| Error                     | Description                                                     |
| ------------------------- | --------------------------------------------------------------- |
| `ERROR_INVALID_SCOPE`     | The `scope` is not `3`.                                         |
