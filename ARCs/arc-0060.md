...
The wallet **MUST** display a message for any signing request. The messae **MUST** be computed dynamically taking as inputs the `metadata.scope`, the `signer`, and the `signData`.
...
This API was designed to enable secure signing of structured data with Algorand wallets. The API:
...
- Is secure by design preventing malicious applications to trick users signing malicious data or in worst cases approve malicious transactions.
...
Users must be aware of what they are signing and for what purpose. The wallet **MUST** always compute a new message dynamically according to the received `data` and `metadata`. This prevents injection of malicious or misleading messages and provide clear scope of the signing action.

Wallets only support known `ScopeTypes` ans schemas. It ensures that signing actions have clear scope and mitigates the risk of producing unexpected signatures.
