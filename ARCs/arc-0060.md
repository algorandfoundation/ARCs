...
This ARC provides a standard API for wallets to sign structured arbitrary data that do not represent and Algorand transaction. This document also includes security constraints to reduce the risk of users being tricked to sign unexpected or malicious messages.
...
ARC-47 introduces the method `algo_templatedLsig` that takes a `LogicSignatureDescription` object and returns the signature of a logic signature program for delegation. This method can be replaced with a call to the `signData()` function where:
...
- `metadata` includes
  ...
  - a new `ScopeType` for logic signatures signing `LSIG`.
...
- if the `metadata[i].scope` is `LSIG`
  - the `ARC60Domain` value of `arbData[i].data ` **MUST** be the string `Program` as specified in the <a href="https://github.com/algorandfoundation/specs/blob/master/dev/crypto.md#domain-separation">Algorand specs</a> and in the <a href="https://github.com/algorand/go-algorand/blob/master/protocol/hash.go#L21">Go type HashID</a>, otherwise the wallet **MUST** reject.
  - the `data` value **MUST** be the hash of the additional parameters of the schema `LogicSignatureDescription`, `values`, and `hash`.
