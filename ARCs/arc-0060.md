...
##### Plain Data JSON Schema

The JSON schema for the plain data. The `StdData` **MUST** be an object of this schema.
...
```json
{
  ...
  "properties": {
    ...
    "bytes": {
      "type": "string",
      "description": "Byte string being signed. Used with ScopeType==PLAIN."
    },
  "required": ["ARC60Domain", "bytes"]
}
```

- The `ARC60Domain` object indicates the domain separator to be used for signing. It **SHOULD** be set to `"arc60"` for simple plain bytes. Algorand transaction domain separators `TX` and `TG` **MUST** be forbidden.
- The `bytes` object is the byte array of data being signed. It is a simple string in case of `ScopeType==PLAIN`, a more complex schema **SHOULD** be used otherwise. It cannot be a valid Algorand transaction. The value of `bytes` **MUST NOT** be prepended with a known domain separator `TX` or `TG`.
...
- the `ARC60Domain` filed must be validated against forbidden values:
  ...
  - the wallet **MUST** reject if `metadata.scope=PLAIN` and `ARC60Domain` field is not set to `"arc60"` or `""`.
...
> For example:"You are about to sign the bytes `ARC60 is awesome` with scope `PLAIN` and message `Sign arbitrary bytes` with the key `1Cz6hgjfllYajnodkJ2hCi+GiYF6ndMHMstH9tJOqlg=`".
