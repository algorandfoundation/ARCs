...
Furthermore, JWTs that are signed using an Algorand account's private key, can be used by clients (such as dApps) to prove the identity of a user and can open up clients to allow limited access to their services using methods such as "scope" (as is suggested in [RFC 8693 section 4.2][rfc-8693-section-42]).
...
In order for EdDSA keys to be encoded as a JWK, the standard [RFC 8037][rfc-8037] introduced a new key type: "OKP" (Octet Key Pair), which describes the use of public key algorithms, namely Ed25519 ([RFC 8032][rfc-8032]), with a JOSE header.
...
As defined in [RFC 7515 section 3][rfc-7515-section-3], the signature is a JSOE header, whose members are the union of the header and the payload, where each part **MUST** be encoded using base64URL ([RFC 4648][rfc-4648-section-5]) encoding, concatenated with a dot (`.`), and then the resulting string is signed using the private key of the Algorand account. 
...
The following example uses the popular [TweetNaCl.js][tweetnacl-js] library for cryptographic signing and verification.

> ⚠️ **NOTE**: At the time of writing, [TweetNaCl.js][tweetnacl-js] was last audited between January-February 2017. The full audit report can be read [here](https://cure53.de/tweetnacl.pdf).
...
[rfc-8693-section-42]: https://datatracker.ietf.org/doc/html/rfc8693#section-4.2
