...
This document requires nothing else, but further semantic meaning is prescribed to these functions in [ARC-10](arc-0010.md#semantic-requirements) which builds off of this one and a few others.
...
Defined as in [ARC-1](./arc-0001.md#interface-algorandaddress):

> An Algorand address is represented by a 58-character base32 string. It includes includes the checksum.

#### Error Standards

`EnableError` follows the same rules as `SignTxnsError` from [ARC-1](./arc-0001.md#error-interface-signtxnserror) and uses the same status error codes.
