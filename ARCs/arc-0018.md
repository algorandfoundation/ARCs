...
If a royalty policy is defined, it **SHOULD** be immutable. 

> The immutability of a policy is left to the implementor but allowing mutability allows a situation where after a sale the royalty policy may be updated from 1% to 100% for example. This would make any further sales have the full payment amount sent to the royalty recipient and the seller would receive nothing.
