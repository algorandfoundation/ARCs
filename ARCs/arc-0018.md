...
```
set_basis(royalty_basis: uint64)
```

Sets the basis points for share of any payment during a `transfer`.

The `royalty_basis` is the share of the payment that should be sent to the `royalty_receiver`.  It is specified in basis points (e.g., 1% is 100).  
...
```
set_allowed_asset(allowed_asset: asset, allowed: bool)
```

The `allowed_asset` argument represents the ASA id that is acceptable for payment. The contract logic **MUST** opt into the asset specified in order to accept them as payment as part of a transfer.
...
```
set_recipient(royalty_receiver: account)
```

The `royalty_receiver` is the address of the account that should receive a partial share of the payment or asset transfer for any [transfer](#transfer) of an asset that uses this contract for Royalty Enforcement 
