...
created: 2022-02-16
adoption-summary: adoption/arc-0018.yaml
requires:
  - 4
  - 20
  - 22
...
> This reasoning for this check is described in [Security Considerations](#security-considerations). It is purposely left to the implementer to decide if it should be checked.
...
Flags the asset as transferable and sets the address that may initiate the transfer request.
...
If any transfer is initiated by an address that is *not* listed as the `auth_address` for this asset ID from this account, the transfer **MUST** be rejected.
...
This getter is *OPTIONAL* unless set_administrator is implemented then *REQUIRED*
...
The exact logic that should determine *if* a transfer should be allowed is left to the implementer.
