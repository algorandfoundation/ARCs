...
The Royalty Collector SHOULD be opted into the full set of assets contained in this list. In the case that an account is not opted into an asset, any transfers where payment is specified for that asset will fail until the account opts into the asset.
...
If this method is called multiple times for the same `royalty_asset` the previous value is overwritten with the new value.

If a transfer is called successfully, the `offer` should be updated or deleted from local state. 
