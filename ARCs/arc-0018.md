...
A Royalty Share consists of a `royalty_receiver` that should receive a Royalty payment and a `royalty_basis` representing some share of the total payment amount. The Royalty Basis **MUST** be specified in terms of [basis points](https://en.wikipedia.org/wiki/Basis_point) of the payment amount.
...
```
set_policy(royalty_basis: uint64, royalty_recipients: account)
```
...
A Royalty Basis **MUST** be immutable, if an application call is made that would overwrite an existing value it **MUST** fail. 
...
The metadata associated to an asset **MAY** conform to any ARC that supports an additional field in the `properties` section specifying the specific information relevant for off-chain applications like wallets or Marketplace dApps.

The fields that should be specified are:

  - `application_id` - The ID of the Royalty Enforcer application
  - `application_address` - The Address of the Royalty Enforcer application
  - `royalty_receiver` - The Address of the recipient of any Royalty Payments
  - `royalty_basis` - The percentage (specified in basis points) of any Royalty Payment
...
"properties":{
    ...
    "arc18":{
        "application_id":123,
        "application_address":"deadbeef",
        "royalty_receiver":"deadbeef",
        "royalty_basis":500
    }
}
//...
...
These fields are provided for convenience only. The ASA parameters and Royalty Policy set in the Royalty Enforcer of the clawback address on the ASA are considered the source of truth if they disagree with these fields.
