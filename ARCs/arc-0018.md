...
[Royalty Policy](#royalty-policy) - The name for the collection of settings that define how royalty payments are collected. 
...
A Royalty Share consists of a `royalty_receiver` that should receive a Royalty payment and a `royalty_basis` representing some share of the total payment amount. The Royalty Share **MUST** be specified in terms of [basis points](https://en.wikipedia.org/wiki/Basis_point) of the payment amount.


### Royalty Enforcer

The Royalty Enforcer is an instance of a contract, an application, that controls the transfer of ASAs subject to the Royalty Policy. 
...
The Royalty Asset is an ASA subject to royalty payment collection and **MUST** be created with the the appropriate mutable and immutable parameters. 
...
This concept is important to this specification because we use the clawback feature to transfer the assets. Without some signal that the current owner is willing to have their assets transferred, it is be possible to transfer the asset without their permission. 
...
```
set_royalty_basis(royalty_basis: uint64)
```

Sets the percentage for royalty payment collection in basis points (e.g., 1% is 100).  

A Royalty Basis **SHOULD** be immutable, if an application call is made that would overwrite an existing value it **SHOULD** fail. 

> TODO: should this be a **MUST**? only should because a contract may want to lower it over time or absolve it completely

> The immutability of a royalty basis is important since mutability introduces the possibility for a situation where, after an initial sale, the royalty policy is updated from 1% to 100% for example. This would make any further sales have the full payment amount sent to the royalty recipient and the seller would receive nothing.
...
```
set_royalty_recipient(royalty_receiver: account)
```
...
Prior to each transfer the the Royalty Enforcer **SHOULD** assert that `AuthAddr == AssetSender` for the Seller (`from` parameter) and `AuthAddr == AssetReceiver` for the Buyer (`to` parameter). 
...
```
get_offer(from: account, royalty_asset: uint64)(address,uint64)
```

> This method is *READ ONLY* as defined in [arc22](./arc-0022.md)

The `from` parameter is the account to inspect for an offer in local state with a key matching the `royalty_asset` passed
...
The `current_offered_amount` is the number of units of this asset currently offered, the value of 0 is passed if no offer is outstanding.

The `current_auth_address` is the authorized address for the current offer, the zero address is passed if no offer is outstanding.
...
```json
{
    ...
    "methods": [
        ...
        {
            "name":"set_royalty_receiver",
            "desc":"Sets the royalty receiver for a given app id",
            "args":[
                {
...
```mermaid
sequenceDiagram
    Alice->>Royalty Enforcer: set_basis with Royalty Basis 
    Alice->>Royalty Enforcer: set_recipient with Royalty Recipient
...
The metadata associated to an asset MAY conform to any ARC that supports an additional field in the `properties` section specifying the Application ID for the Royalty Enforcer Contract.  This allows off chain applications like wallets or Marketplace dApps to retrieve the relevant policy information.
