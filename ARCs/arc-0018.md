...
> All the Royalty Collector SHOULD be opted into the full set of assets contained in this list but MAY NOT be checked ahead of time. In the case that an account is not opted into an asset, any transfers where payment is specified for that asset will fail until the account opts into the asset.


##### Transfer: 

    transfer(royalty_asset: asset, from: account, to: account, royalty_collector: account, payment: transaction)
    transfer(royalty_asset: asset, from: account, to: account, royalty_collector: account, payment: transaction, payment_asset: asset)
...
The transfer call MUST be part of a group with a size of 2 (app call + payment/asset transfer)
...
#### Offer:
    ...
    offer(royalty_asset: asset, authorized_address: account)
...
##### Move 
    ...
    move(royalty_asset: asset, to: account)
...
The `royalty_asset` parameter is the asset being transferred without applying the Royalty Enforcement logic.
