...
Existing ASA willing to conform to this ARC **MUST** provide the ASA Circulating
Supply App ID with an [ARC-2](./arc-0002.md) message, either:

- (**RECOMMENDED**)In the [ASA metadata](#asa-circulating-supply-app-id-on-asa-metadata-registry)
on the ASA Metadata Registry, or
- (**NOT RECOMMENDED**) [Directly](#asa-circulating-supply-app-id-in-arc-2-note-field).

As described in the subsections below.

Clients **MUST** inspect existing ASA `AssetConfig` transaction history to discover
the [ARC-2](./arc-0002.md) message.

Clients **SHOULD** optimistically inspect the ASA Metadata Registry first to discover
the Circulating Supply App ID (to avoid scanning the transaction history).
