...
```
However the above method may prevent issues with versioning smart contracts.
If a method changes in a way that is compatible with the previous version,
the method selector still changes, so dependencies with calling contracts are broken.
The contract then needs to add multiple selectors, defeating the space saving aim.
If the contract derives from an HLL, where arguments are tuples derived from structs,
then modifications to the structures (such as adding a new optional field) changes 
the above selector. 

For these reasons, manually chosen selectors can also be specified, to override the 
...
interface Method {
  ...
  desc?: string;
  /** Optional, user-friendly selector */
  selector?: string;
  /** The arguments of the method, in order */
  args: Array<{
    /** The type of the argument */
    type: string;
    /** Type and validation hints **/
    hints: string;
...
```json
{
  ...
  "args": [
    { "type": "uint64", "hints":["Max(10000)"], "name": "a", "desc": "The first term to add" },
    { "type": "uint64", "hints":["Max(10000)"], "name": "b", "desc": "The second term to add" }
  ],
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "args": [
        { "type": "uint64",  "hints":["Max(10000)"],"name": "a", "desc": "The first factor to multiply" },
        { "type": "uint64",  "hints":["Max(10000)"],"name": "b", "desc": "The second factor to multiply" }
      ],
...
descriptions for each of the methods in the Contract and
the State the contract supports. Inclusion of State also informs
of the StateSchema (num byte slices, num global ints, etc).
...
interface Variable {
  ...
  name: string;
  /** Optional base64 byte array state key**/
  bytes_name: string;
...
Variable name must also satisfy the same regular expression as Contract name for HLL support. 
Variables representing byte array keys can use the bytes_name alias to supply a base64 key.
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "returns": { "type": "uint64",  "hints":["Max(20000)"], "desc": "The product of a and b" }
    }
  ]
}
```
...
* `uint<N>`: An `N`-bit unsigned integer, where N is 8,16,32 or 64.
* `ubigint`: A biginteger identical to the TEAL type - a byte array that may be 0 to 512 bits wide.
* `int<N>`: An `N`-bit signed integer, where N is 8,16,32 or 64, where negative values are represented using 2's complement
* `bigint`: A signed biginteger where negative values are 512 bit 2's complement.
* `byte`: An alias for `uint8`.
* `sbyte`: An alias for `int8`.
* `datetime`: Signed 32 bit unix epoch time, where negative is two's complement.
...
* `(name1:T1,name2:T2,…,nameN:TN)`: A named tuple of the types `T1`, `T2`, …, `TN`, `N >= 1`, for use in contract.json to help representation of structs/objects from HLLs.
...
### Type hints
The following hints to further describe an argument or return type are available. 
Hints are arrays of functions in an argument description:

* Max(N) - the maximum decimal or hexadecimal ('0x...') value,N, this integer can contain
* Min(N) - the minimum decimal or hexadecimal ('0x...') value,N, this integer can contain
* ...
