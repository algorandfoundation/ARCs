...
restricted to by the regular expression `[A-Za-z0-9_]+`. The preferred
naming scheme for methods is camelCase.
...
but there must be no more than one method with each name.

If either of these methods is elided, an ARC-4 app **MUST** allow
opt-in or close-out transactions with no arguments.
