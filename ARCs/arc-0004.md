...
`_closeOut`. These methods describe how callers can opt-in to, or
...
In the absense of such a method, ARC-4 Contracts **SHOULD** allow
accounts to perform the corresponding action with no arguments, or by
setting the associated OnCompletion value while calling a method of
the contract.
...
schema. Opt-in is unnessary if the app has no local schema.  However,
...
> the amount of recipient of a pay transaction be specifed? A
