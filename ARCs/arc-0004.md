...
on an Algorand _app_ (stateful contract).
...
dapps, to properly encode call transactions based on a simple
description of the interface. Further, explorers will be able to show
details of these method invocations.
...
externally callable. In the future, applications may be able to invoke
...
contain an open parenthesis or whitespace. For the benefit of univeral
...
properly encode arguments and call methods, as well as offer users a
rich description for a method and its arguments. This description can
populate UIs in dapps, wallets, and explorers with description of
parameters, as well as populate information about methods in IDEs for
HLLs.
...
concrete implementation. All methods in an contract must be unique
...
reserved for use by this, or suceeding specifications.

A Contract may contain two special methods, `_optIn` and
`_closeOut`. These methods describe how callers should opt-in to, or
close-out from, such apps.  They may have any arguments or return
values, but there must be no more than one method with each name.

In the absense of such a method, ARC-4 Apps **SHOULD** allow accounts to
perform the corresponding action with no arguments, or by setting the
associated OnComplete value while calling a method of the contract.
...
methods `_optIn` and `_closeOut`, the first argument **MUST** be
empty. Instead, the OnCompletion (`apan`) field of the application
call transaction is set appropriately. For all other methods, `apan`
is set to NoOpOC (`0`).
...
method implementation. Only a single value may be returned by a method
and it will be encoded as defined in the Encoding section, and emitted
from TEAL (v5) with the `log` opcode.

_Exact conventional use of `log` TBD. Log will also allow "events"_
...
1. **MUST** Examine `txn ApplicationArgs 0` to identify the selector of
the method being invoked. If the contract does not implement a method
with that selector, the call **MUST** fail.
...
the 14th one, it must decode `txn ApplicationArgs 15` as a tuple to
access the arguments contained in it.

4. If the method is non-void, the application **MUST** be encode the
...
2. Submit this transaction and wait until it successfully commits to
   the blockchain.
3. From the submission information about that transaction, decode the
   return value, if any, from the ApplyData's log information.

Clients **MAY** ignore the return value.

### Calling a Method from Another Contract

It is currently impossible for a AVM code to invoke another AVM
app. This is being contemplated, however, and when it becomes
poossible we will need to describe how to prepare the arguments for a
such a call from AVM code, rather than as transaction encoding.
...
that type have the same length, irrepective of their actual value.
...
This document describes argument and return value encoding only.
There are several other aspects to invoking an Algorand app properly
that require standardization to improve high-level access to
contracts.

### Opt In, Close Out, and Clear State.

An ARC-4 app **MUST** allow any account to opt-in and close-out, using
an application call transaction with no arguments an OnComplete equal
to . Clients **MAY** determine that such an opt-in is unnecessary by
querying the blockchain for the app's schema. Opt-in is unnessary if
the app has no local schema.  However, even if a local schema is
present, some methods may be callable without opt-in. Clients **MAY**
attempt method calls without opt-in.
...
an ARC-4 app **MUST** fail the transaction, or interpret the arguments
as an invocation as described in the rest of this document, to perform
along with the opt-in or close-out.

### Foreign apps, asas, and accounts

Some AVM opcodes require specific values to be placed in the "foreign
arrays" of the application call transaction. Three special types,
`account`, `asset`, `application` allow methods to describe these
requirements. To encode method calls with such types, place the value
in question in the Accounts (`apat`), Foreign Assets (`apas`), or
Foreign Assets (`apfa`) arrays, and encode a uint8 referring to that
value in the normal location for the value. For the Accounts and
Applications array, a 0 implicitly refers to the Sender and the called
application respectively, and the passed values begin at 1.

When passing addresses, ASAs, or apps that are _not_ required to be
accessed by such opcodes, ARC-4 Apps **SHOULD** use the base types for
passing these types: `address` and `uint64`.

### Transaction groups.

Some apps require that they are invoked as part of a larger
transaction group, containing specific additional transactions. Future
versions of this document may describe how a method signature could
...
example, they should use `txns` to examine the previous index for a
required `pay` transaction, rather than hardcode an index with `txn`.

> Comments on this section are particularly welcome. A strawman
> proposal is to introduce a new type for each transaction type. The
> appearance of such a transaction type would imply that such a
> transaction must appear in the transaction group, in the order of
> their appearance in the call. Unlike "foreign" references, these
> special types would not be encoded as a small integer "pointing" to
> the associated object, as allowing such references would create
> opportunities for multiple application calls to point to the same
> transaction in the group, which is undesirable. Instead, the
> locations of the transactions would be implied entirely by the
...
> a signature specify the setting of the the size or recipient of a pay
> transaction? A specific, important instance of this problem: How can
> logicsigs on these transactions be provided in the JSON interface?
