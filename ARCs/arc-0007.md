...
Defined as in [ARC-1](./arc-0001.md#interface-signedtxnstr):
...
`PostTxnsError` follows the same rules as `SignTxnsError` from [ARC-1](./arc-0001.md#error-interface-signtxnserror) and uses the same status codes as well as the following status codes:
...
* If unsuccessful, `postTxns` **MUST** reject the promise with an error `err` of type `PostTxnsError` such that:
  * `err.code=4400` if there was a failure sending the transactions or a code as specified in [ARC-1](./arc-0001.md#error-standards) if the user or function disallowed posting the transactions.
