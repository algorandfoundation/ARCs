...
In order to receive a message, an account **MUST** create an application **MUST** that follow the following convention:

- A Local State named `public_key` **MUST** containing a public account `Ed25519-Address`
  - If the account is a public account, `public_key` **MAY** be equal to the account's Address as `public_key`, however it is **NOT RECOMMENDED**.
  > If `public_key` = account's Address, the decryption will be done by using the private key of the account directly. 
- A Box `inbox` with:
