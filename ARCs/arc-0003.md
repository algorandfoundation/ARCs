...
* *Asset URL* (`au`): a URI that **SHOULD** point to a JSON Metadata file. If the URI does not point to a JSON Metadata file, it is assumed the JSON Metadata is stored in the note field of the initial `acfg` transaction for this asset. This URI as well as any URI in the JSON Metadata file:
