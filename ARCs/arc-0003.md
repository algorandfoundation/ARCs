...
This document introduce conventions for the parameters of Algorand Standard Assets (ASAs) for fungible tokens and non-fungible tokens (NFTs). An ASA is a digital proof, which may be tokenized for various purposes on the Algorand Network. Fungible tokens are digital assets which can be transfered between addresses via a blockchain network, and may operate as a type of currency or utility asset. By contrast, NFTs are decentralized data certificates, which authenticate both virtual and real world assets, such as a photographs, paintings, or music, on a blockchain.
