...
`account_freeze` prevents a specific account from transferring a Smart ASA.
...
whether any account (respectively `freeze_account`) can transfer `freeze_asset`.
A `false` value indicates that the transfer will be rejected.
