...
> A `uint8` is enough as a `page` index since `(MAX_METADATA_SIZE // PAGE_SIZE)=32`.
...
To get the Metadata Header Hash for an ASA:

- The ASA **MUST** still _exist_, and

- The Asset Metadata Box **MUST** _exist_.

The Metadata Header Hash **MUST** be returned according to the [Metadata Hash Computation](#metadata-hash-computation).
