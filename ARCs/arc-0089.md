...
##### ASA Metadata Registry MBR

The Minimum Balance Requirement (MBR) for the ASA Metadata Registry Application Account
**MUST** be provided _before_ enabling the creation of any Asset Metadata Box.
...
To get the Asset Metadata Box MBR Delta for an ASA:
...
- If the Asset Metadata Box related to the ASA _does not exist_, `flat_mbr != 0` and then:
    - The returned _sign_ **MUST** be _positive_ (`1`), and
    - `delta_size = box_name_size + metadata_size`.
