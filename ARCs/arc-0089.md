...
- The maximum [ARC-4](./arc-0004.md) _return type_ encoding overhead bytes depends on the
...
1. The ASA Creator Address creates an ASA as follows:
   - The _Asset URL_ (`au`) field is set to `algorand://app/<singleton_arc89_app_id>?box=#arc3`,
   - If the Asset Metadata is [_immutable_](#metadata-immutability), the _Asset Metadata
...
> The compliance fragment for [ARC-3](./arc-0003.md) **MUST NOT** contain additional
> elements (i.e., `#arc3+89` is not allowed), see [ARC-90 compliance fragment](./arc-0090.md#compliance-fragment)
> section for details.
...
> The TestNet deployments uses `testnet` as `netlabel` for the [ARC-90] network authority
> selector, resulting in the following _partial_ Asset URL:
...
If the ASA is declared as [ARC-3](./arc-0003.md) compliant, the _Asset Name_ (`an`)
or the _Asset URL_ (`au`) **MUST** comply with the [_ARC-3 ASA Parameters Conventions_](./arc-0003.md#asa-parameters-conventions).
...
- The top-level key **MUST** exist, and
