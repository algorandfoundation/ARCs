...
if and only if `metadata_size ≤ MAX_SHORT_METADATA_SIZE`, and `False` otherwise.
...
A `page` **MUST** be identified by a 0-based index (`uint8`).

> A `uint8` is enough as a `page` index since `ceil(MAX_METADATA_SIZE/PAGE_SIZE) = 33`.
...
1. The ASA Creator Address creates an ASA as follows:
   ...
   using the [Pre-compute Metadata Identifiers getter](#precompute-metadata-identifiers),
   the defined Metadata Flags and the Metadata (raw bytes),
...
metadata does not exceed `MAX_METADATA_SIZE`. Existing ASAs are not identified as
a [ARC-89 native ASA](#arc-89-native-asa).
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "events": [
        {
          "name": "Arc89MetadataUpdatedFlags",
    ...
    },
    {
      ...
      "readonly": true,
      "args": [],
      "returns": { "type": "(uint16,uint16,uint16,uint16,uint64,uint64)", "desc": "The first value is the HEADER_SIZE, the second value is the MAX_METADATA_SIZE, the third value is the MAX_SHORT_METADATA_SIZE, the fourth value is the PAGE_SIZE, the fifth value is the FLAT_MBR, the sixth value is the BYTE_MBR" }
    },
    {
    ...
    },
    {
      ...
      "args": [
        ...
        { "type": "uint8", "name": "key_type", "desc": "The JSON key type (0: JSON String, 1: JSON Uint64, 2: JSON Object)" }
      ],
    ...
    {
      "name": "arc89_validate_metadata_identifiers",
      "desc": "Validate expected Metadata Identifiers for an ASA, given the ASA field and Metadata size",
      "readonly": true,
      "args": [
        { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to validate the Metadata Identifiers for" },
        { "type": "uint16", "name": "metadata_size", "desc": "The Asset Metadata byte size at creation" },
        { "type": "byte", "name": "expected_identifiers", "desc": "The expected Metadata Identifiers bits" }
      ],
      "returns": { "type": "bool", "desc": "Returns True if the ASA is identified with the expected Metadata Identifiers bits" }
    }
  ]
}
```
...
An `Arc89MetadataCreated` event **MUST** be emitted.
...
> ⚠️ WARNING: The extra payload can be provided by any caller, not necessarily the
> ASA Manager Address, but the header call (create or replace) authorization is still
> restricted to the ASA Manager Address. This enables third-party services to provide
> Asset Metadata on behalf of the ASA Manager Address, while the ASA Manager Address
> authorizes the Asset Metadata create or update.
...
An `Arc89MetadataUpdatedFlags` event **MUST** be emitted.
...
- The fifth value is the `FLAT_MBR` (`uint64`),

- The sixth value is the `BYTE_MBR` (`uint64`).
...
The Metadata Hash **MUST** be returned.
...
- The `key_type` (`uint8`) **MUST** be either `0` (JSON String), `1` (JSON Uint64),
or `2` (JSON Object), and
...
##### Precompute Metadata Identifiers

To precompute the Metadata Identifiers for an ASA:

- The `asset_name` (`str`) **MUST** be a valid ASA Asset Name (`an`) field, and

- The `asset_unit` (`str`) **MUST** be a valid ASA Unit Name (`un`) field, and

- The `asset_url` (`str`) **MUST** be a valid ASA URL (`au`) field, and

- The `metadata_size` (`uint16`) **MUST NOT** exceed the `MAX_METADATA_SIZE`.

The Metadata Identifiers **MUST** be returned as a tuple `(bool,bool,bool)` as if
the Asset Metadata of an ASA with the same fields was created.

##### Validate Metadata Identifiers

To validate the Metadata Identifiers for an ASA:
...
The method **MUST** return `True` if the Asset Metadata for the ASA would have been
identified with the `expected_identifiers` (`byte`) at creation, `False` otherwise.
