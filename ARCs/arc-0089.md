...
| Test Net | `SGO1GKSzyE7IEPItTxCByw9x8FmnrCDexi9/cOUJOiI=` | `752407891` | `QYK5DXJ27Y7WIWUJMP3FFOTEU56L4KTRP4CY2GAKRXZHHKLNWV6M7JLYJM` |
...
| [Metadata Flags](#metadata-flags)             | Header |       Yes        |   `byte`   |     `1`     |            `1`            |
| [Metadata Hash](#metadata-hash)               | Header |  No (Recursive)  | `byte[32]` |     `2`     |           `32`            |
| [Last Modified Round](#last-modified-round)   | Header |        No        |  `uint64`  |    `34`     |            `8`            |
| [Metadata](#metadata)                         | Body   |       Yes        |  `byte[]`  |    `42`     | up to `MAX_METADATA_SIZE` |
...
by the ASA Manager Address on creation or at any time.

The Metadata Flags are defined as follows:

|  BIT  | DESCRIPTION                                          | DEFAULT | STATE TRANSITION     |
|:-----:|:-----------------------------------------------------|:-------:|:---------------------|
| `LSB` | [ARC-20 Smart ASA](#smart-asa)                       | `False` | Two-ways, any time   |
|  `1`  | [ARC-62 Circulating Supply](#asa-circulating-supply) | `False` | Two-ways, any time   |
|  `2`  | Custom, should be reserved for future ARCs           | `False` | Two-ways, any time   |
|  `3`  | Custom, should be reserved for future ARCs           | `False` | Two-ways, any time   |
|  `4`  | [ARC-3 Compliant](#arc-3-compliance)                 | `False` | One-way, at creation |
|  `5`  | [ARC-89 Native ASA](#arc-89-native-asa)              | `False` | One-way, at creation |
|  `6`  | Custom, should be reserved for future ARCs           | `False` | One-way, any time    |
| `MSB` | [Metadata Immutability](#metadata-immutability)      | `False` | One-way, any time    |

The `MSB` is the leftmost bit in the byte stored in Asset Metadata Box.

The bits `2, 3, 6` are reserved for future ARCs and **MUST** be initialized to `False`.

The state transition of bits:

- `LSB ... 3` are _two-ways_ (reversible) and can be set at any time,

- `4, 5` are _one-way_ (irreversible) and can be set at creation time only,

- `6, MSB` are _one-way_ (irreversible) and can be set at any time.
...
- The [ARC-4](./arc-0004.md) method selector size `ARC4_METHOD_SELECTOR_SIZE` (`4`
bytes);

- The available payload for the method `arc89_create_metadata(uint64,byte,uint16,byte[],pay)`
(`FIRST_PAYLOAD_MAX_SIZE = MAX_ARG_SIZE - (ARC4_METHOD_SELECTOR_SIZE + 8 + 1 + 2 + 2 + 0) = 2031` bytes),
...
(`REPLACE_PAYLOAD_MAX_SIZE = MAX_ARG_SIZE - (ARC4_METHOD_SELECTOR_SIZE + 8 + 2 + 2 = 2032)` bytes);

> Refer to the [ARC-4 Interface](#arc-4-interface) section for details about the
> method signatures.

Therefore, `MAX_METADATA_SIZE = FIRST_PAYLOAD_MAX_SIZE + 14 * EXTRA_PAYLOAD_MAX_SIZE = 30507`
bytes.
...
- `Metadata Flags` is the 1-byte encoding of the [Metadata Flags](#metadata-flags) (`byte`);
...
The _Asset URL_ (`au`) **MUST** begin with the _partial_ [ARC-90](./arc-0090.md) URI:
...
`algorand://app/<singleton_arc89_app_id>?box=#arc<A>+<B>+<C>...`

where `<A>`, `<B>`, `<C>`, etc. are the ARC numbers of the compliance fragments.

The [Native ARC-89 ASA Flag](#arc-89-native-asa) **MUST** be set to `True`.
...
> the _complete_ [ARC-90](./arc-0090.md) _Asset Metadata URI_.
...
for the _Asset Name_ (`an`) and the _Asset URL_ (`au`) fields.
  ...
  the _partial_ [ARC-90](./arc-0090.md) URI would be: `algorand://app/<singleton_arc89_app_id>?box=#arc3`
...
- The [ARC-3 Compliant Flag](#arc-3-compliant) **MUST** be set to `True`.
...
To get the [ARC-90](./arc-0090.md) _Asset Metadata URI_, clients **SHALL** complete
...
Clients **MUST** encode the Asset Metadata Box Name with `base64url` in [ARC-90](./arc-0090.md)
...
> | Asset ID (`uint64`) | 8-byte big-endian (hex) | Algod `/box?name=` (`base64`) | ARC-90 `box=` (`base64url`, no padding) |
> |--------------------:|:-----------------------:|:-----------------------------:|:---------------------------------------:|
...
By fixing a _singleton_ application per Algorand network and using a partial [ARC-90](./arc-0090.md)
...
as an [ARC-89 native ASA](#arc-89-native-asa).
  ...
  "methods": [
    ...
    },
    {
      ...
      "args": [
        ...
        { "type": "uint8", "name": "flag", "desc": "The reversible flag index to set. WARNING: must be 0 ... 3" },
    ...
    },
    {
      ...
      "args": [
        { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to set the Metadata Flag for" }, 
        { "type": "uint8", "name": "flag", "desc": "The irreversible flag index to set. WARNING: must be 6, 7" }
      ],
      "events": [
        {
          "name": "Arc89MetadataUpdated",
    ...
    },
    {
      ...
      "readonly": true,
      "args": [],
      "returns": { "type": "(uint16,uint16,uint16,uint16,uint16,uint16,uint16,uint64,uint64)", "desc": "Tuple of (HEADER_SIZE, MAX_METADATA_SIZE, SHORT_METADATA_SIZE, PAGE_SIZE, FIRST_PAYLOAD_MAX_SIZE, EXTRA_PAYLOAD_MAX_SIZE, REPLACE_PAYLOAD_MAX_SIZE, FLAT_MBR, BYTE_MBR)" }
    },
    {
    ...
    },
    {
       ...
       "desc": "Return the UTF‑8 string value for a top‑level JSON key from short Metadata for an ASA; errors if the key is not a string or does not exist",
       "readonly": true,
       "args": [
    ...
    }, 
    {
       ...
       "desc": "Return the uint64 value for a top‑level JSON key from short Metadata for an ASA; errors if the key is not an uint64 or does not exist",
       "readonly": true,
       "args": [
          { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to get the key value for" },
    ...
    },
    {
       ...
       "desc": "Return the UTF-8 object value for a top‑level JSON key from short Metadata for an ASA; errors if the key is not an object or does not exist",
       "readonly": true,
       "args": [
          { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to get the key value for" },
...
If the ASA is declared as [ARC-89 Native ASA](#arc-89-native-asa) the _Asset URL_
(`au`) **MUST** comply with the specified [Asset Metadata URI](#asset-metadata-uri)
with no `#arc` fragment validation.
...
The _reversible_ `flag` (`uint8`) **MUST** be either `0, 1, 2, 3`.

The `flag` **MUST** be set to the provided `value` (`bool`).
...
The _irreversible_ `flag` (`uint8`) **MUST** be either `6, 7`.

The `flag` **MUST** be set to `True` (idempotent).
...
> ⚠️ WARNING: flags 4, 5 are set only at creation time, for further details refer
> to the [Metadata Flags section](#metadata-flags).
...
- The third value is the `SHORT_METADATA_SIZE` (`uint16`),`

- The fourth value is the `PAGE_SIZE` (`uint16`),

- The fifth value is the `FIRST_PAYLOAD_MAX_SIZE` (`uint16`),

- The sixth value is the `EXTRA_PAYLOAD_MAX_SIZE` (`uint16`),

- The seventh value is the `REPLACE_PAYLOAD_MAX_SIZE` (`uint16`),

- The eighth value is the `FLAT_MBR` (`uint64`),

- The nineth value is the `BYTE_MBR` (`uint64`).
...
The Metadata Header **MUST** be returned as a tuple `(byte,byte,byte[32],uint64)`, where:
...
- The third value (`byte[32]`) is the [Metadata Hash](#metadata-hash).

- The fourth value (`uint64`) is the [Last Modified Round](#last-modified-round),
...
- The value length **MUST NOT** exceed `PAGE_SIZE`.

The top-level key's value (`string`) from the JSON Metadata object **MUST** be returned.

> ⚠️ WARNING: This getter does not provide pagination or truncation of the returned
> value.

> ⚠️ WARNING: A missing top-level key causes a runtime error.
...
The top-level key's value (`uint64`) from the JSON Metadata object **MUST** be returned.
...
### [ARC-28](./arc-0028.md) Events

| ARC-28 EVENT SIGNATURE                                          | 4-BYTE SELECTOR (HEX) |
|-----------------------------------------------------------------|-----------------------|
| `Arc89MetadataUpdated(uint64,uint64,uint64,byte,bool,byte[32])` | `f577c01f`            |
...
- Metadata Body (`byte[]`): [ARC-3](./arc-0003.md) JSON Metadata.
