...
operations with a big-endian bitmask.

The bits in the big-endian Metadata Header are defined as follows:
...
- The `metadata_header` (`byte`) size **MUST** be `1`.

The [Metadata Header](#metadata-header) **MUST** be initialized with the `metadata_header`
value.
...
The [Metadata Header](#metadata-header) **MUST** be set to `metadata_header` (`byte`)
value.
