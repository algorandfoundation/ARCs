...
> The Minimum Balance Requirement (MBR) of a Box is calculated as:
>
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "returns": { "type": "(uint8,uint64)", "desc": "Null (0), positive (1), or negative (255) MBR Delta amount (microALGO)" }
    }
  ]
}
```
...
The Asset Metadata Box _MBR Delta_ is a tuple of two elements, encoding:

- The _sign_ (`uint8`), either null (`0`), positive (`1`), or negative (`255`);

- The _amount_ (`uint64`) of MBR, expressed in microALGO.

These values depend on the _existence_ of the Asset Metadata Box for the ASA and
the relative byte sizes (`delta_size`) between the _new_ Asset Metadata (`metadata_size`)
