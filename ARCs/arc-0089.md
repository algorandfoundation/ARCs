...
> specifically:
> 
> - If the Metadata size at creation time is less than or equal to `MAX_SHORT_METADATA_SIZE`.
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "desc": "Set a reversible Asset Metadata Flags, restricted to the ASA Manager Address",
      "args": [
    ...
    },
    {
      ...
      "desc": "Set an irreversible Asset Metadata Flags, restricted to the ASA Manager Address",
      "args": [
        { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to set the Metadata Flags for" }, 
...
The _reversible_ `flag` (`uint8`) **MUST** be either `LSB ... 3`.
...
The _irreversible_ `flag` (`uint8`) **MUST** be either `6 ... MSB`.

The `flag` **MUST NOT** be set if it is already `True`.
...
- `Page Index` is encoded as single `uint8`,
