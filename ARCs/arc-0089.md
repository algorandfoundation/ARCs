...
The Metadata Hash (`byte[32]`) is a 256-bit hash defined in the [Asset Metadata Hash
section](#asset-metadata-hash).
...
The _Asset Metadata Hash_ (`am`) **MUST** be computed as follows:
...
The ASA Creator **MUST** pre-compute the _Asset Metadata Hash_ (`am`) field according
to this specification and used it as hash-lock invariant on ASA creation, if:

- The ASA _is not_ [ARC-3 compliant](#arc-3-compliance), and
- Asset Metadata will be set as [_immutable_](#metadata-immutability) at creation.

Otherwise, the _Asset Metadata Hash_ (`am`) field of the ASA **MUST NOT** be set
(i.e., set to zero).

The ASA Metadata Registry **MUST** validate the _Asset Metadata Hash_ (`am`) field
of the ASA and reject the Asset Metadata creation on validation failure, if:

- The _Asset Metadata Hash_ (`am`) is set, and
- The ASA _is not_ [ARC-3 compliant](#arc-3-compliant).

> The Metadata Identifiers are set by the ASA Metadata Registry on creation, therefore
...
> 1. If the ASA is [ARC-3](#arc-3-compliance) compliant,
...
- The ASA **MUST** comply with the [_ARC-3 ASA Parameters Conventions_](./arc-0003.md#asa-parameters-conventions)
for the _Asset Metadata Hash_ (`am`) field, if:

  - The ASA is an [ARC-89 native ASA](#arc-89-native-asa),
  - The Asset Metadata are set as [_immutable_](#metadata-immutability) at creation. 
...
1. The ASA Creator Address creates an ASA as follows:
   ...
   using the pre-computed Metadata Identifiers and the defined Metadata Flags and
   Metadata (raw bytes),
