...
for the _Asset Name_ (`an`) and the _Asset URL_ (`au`) fields.
  ...
  the _partial_ [ARC-82](./arc-0082.md) URI would be: `algorand://app/<singleton_arc89_app_id>?box=#arc89+arc3`
...
1. The ASA Creator Address creates an ASA as follows:
   - The _Asset URL_ (`au`) field is set to `algorand://app/<singleton_arc89_app_id>?box=#arc89+arc3`,
   - If the Asset Metadata is [_immutable_](#metadata-immutability), the _Asset Metadata
...
> `/box?name=` to be `base64` encoded, which is equal to the `base64url` for this
> specific use case.
...
> `12345` would be:
>
> `algorand://app/<singleton_arc89_app_id>?box=AAAAAAAAMDk=#arc89+arc3`
...
```json
{
  ...
  "methods": [
    ...
    },
    {
      ...
      "returns": { "type": "uint64", "desc": "Timestamp of Metadata Flags setting" }
    },
    {
    ...
    },
    {
      ...
      "args": [
        { "type": "uint64", "name": "asset_id", "desc": "The Asset ID to set the Asset Metadata slice for" },
      ...
      "returns": { "type": "uint64", "desc": "Timestamp of slice setting" }
    },
    {
    ...
    },
    {
      ...
      "returns": { "type": "bool", "desc": "Asset Metadata for the ASA is short" }
    },
    {
...
The Asset URL (`au`) **MUST** be validated:
  ...
  - If it conforms to the [Asset URL section](#asset-url) specification, the [ARC-89
    ...
    - If it ends with the `+arc3` suffix the [ARC-3 Compliant Identifier](#arc-3-compliant)
    **MUST** be set to `True`.
...
To provide extra payload to concatenate to Asset Metadata creation or replace for
an ASA:
...
> 
> **Example:** Creating and updating different Assets Metadata in same Group Transaction
>
> ```plain
...
The method **MUST** return the value of the [Smart ASA](#smart-asa) flag.
...
```plain
SHA-512/256("arc0089/page" || Asset ID big-endian 8-bytes || Page byte || Page bytes)
```
...
> ⚠️ WARNING: This getter does not provide pagination of the returned value.
...
endpoint to get the _Asset URL_ field (`url`) from the response and drops the `#arc89+arc3`
...
Given the _Asset Metadata URI_ `algorand://app/<singleton_arc89_app_id>?box=<base64_encoded_asset_id>#arc89+arc3`,
the client:
