...
author:
  - Patrick Bennett / TxnLab Inc. (@pbennett)
...
sub-category: ASA
created: 2021-01-23
adoption-summary: adoption/arc-0019.yaml
superseded-by: 89
---
...
1. **An "ipfscid" _template_ argument in place of the normal CID.**
...
1. `<multicodec content-type name>` **MUST** be an IPFS multicodec name. Client implementations **MUST** support 'raw' or 'dag-pb'. Other codecs **SHOULD** be supported but are beyond the scope of this proposal.
1. `<field name>` **MUST** be 'reserve'.
...
1. `<hash type>` **MUST** be the multihash hash function type (as defined in <https://multiformats.io/multihash/>). Client implementations **MUST** support 'sha2-256' and **SHOULD** support future hash types when introduced by IPFS.
...
An example was pushed to TestNet, converting from an existing ARC-3 MainNet ASA (asset ID 560421434, <https://lora.algokit.io/mainnet/asset/560421434>)
...
The resulting ASA can be seen on <https://lora.algokit.io/testnet/asset/66753108>
