...
- User and Verifier **MUST** communicate over SSL/TLS encrypted channels;
- The User **MUST** register to the Verifier with *PKa*;
- The User authenticates with *PKa*;
- The Verifier **MUST** keeps the mapping between the User and *PKa*;
- For each *PKa* the Verifier **MUST** generate a unique challenge;
...
- The account secret key *SKa* **SHOULD** be stored in a Wallet;
- Users **MUST NOT** authenticate with Algorand <a href="https://developer.algorand.org/docs/get-details/dapps/smart-contracts/smartsigs/modes/#logic-signatures">LogicSigs</a> or <a href="https://developer.algorand.org/docs/get-details/dapps/smart-contracts/apps/#application-account">Application accounts</a>.

### Authentication Mechanism

The ARC-0031 authentication works as follows: the User sends an authentication request to the Verifier with the Algorand account <*PKa, SKa*> stored in Wallet. The Verifier checks PKa and generates a new authentication challenge.

> A reference implementation **SHOULD** store by the Verifier the mapping between the User and the *PKa*.

 The Verifier sends the challenge back to the User to generate a proof of ownership (POWN). The User verifies the challenge origin and eventually signs the challenge with the signing method of the Waller. It the User confirms the signature, the Wallet generates a POWN. The result is sent back to the Verifier which checks the validity of the POWN and eventually authenticates the User.

> A reference implementation **SHOULD** implement a method to abort the authentication in case the User or the Verifier reject the challenge or the POWN respectively.
>
>ARC-0031 does not handle authentication session, which is considered as **OPTIONAL**. The implementation **MAY** implement sessions using traditional approaches based on session tokens exchanged between Verifier and User.

Wallets **MUST** check that the Verifier specified within the challenge (called *origin*) matches with the expected Verifier declared by the User (this check protects Users from <a href="https://en.wikipedia.org/wiki/Man-in-the-middle_attack">man-in-the-middle attacks</a>).
...
The ARC-0031 authentication mechanism **MUST** follow these steps:

1. The User **MUST** connect the *PKa* to the Waller and initiate an authentication to the Verifier;
2. The Verifier **MUST** check the *PKa* with the User, generate a unique `challenge` and send it back to the User;
3. The User **MUST** sign the `challenge` with the sign method of the Wallet;
4. The Wallet **MUST** check the `challenge` format and origin with the given *PKa* and Verifier;
5. If the origin is correct, the User **MUST** approve the signature, retrieve the `POWN = <PKa, Sig(challenge)>`, and send it to the Verifier;
6. The Verifier **MUST** check the `POWN` with the User's *PKa* and known `challenge`;
7. If the signature is valid, the User is authenticated.

ARC-0031 defines a standardized challenge, called *Authentication Message*.
