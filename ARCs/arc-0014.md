...
After authentication, the Verifier **SHOULD** exchange a `session-id` with the User, such as a cookie or a JWT. The way `session-id`s are implemented is an implementation choice. However, to comply with the ARC-14, it **MUST** include the following information:
...
For example, an ARC-14 compliant JWT might be implemented as follows:
