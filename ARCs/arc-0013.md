...
The dapp will, itself, listen to broadcasted responses from any providers. Below is a simple diagram on the flow of data:
...
* `params.providerId`:
  ...
  - **MUST** uniquely identify the provider
* `params.stxns`:
  - **MAY** have an item that is a base 64 encoded of a signed transaction
  - **MAY** have an item that is null if the transaction at the same index of request `txns` item was not signed by the provider
  - **MAY** be empty
* `reference`:
