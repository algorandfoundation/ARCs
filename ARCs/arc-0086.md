...
Once the xGov Registry confirms the acknowledgement on block `h ≠ 0` for the address `a`, it acquires an xGov status `(a, h)` and is considered an xGov.

The xGov status **MAY** be revoked from address `a`, either by themself (unsubscribing from the xGov Registry) or by the xGov Registry rules.

The xGov status `(a, h)` of an address `a` **SHOULD** be persisted on the xGov Registry state.

If the xGov status `(a, h)` is revoked it **SHOULD NOT** be deleted from xGov Registry state, instead it **SHOULD** be marked as _revoked_ with `h = 0`.

### xGov Voting Power

Given a governance period `(Bi, Bf)`, an xGov `(a, h)` is _eligible_ to acquire voting
power for that period if and only if:

- `h ≠ 0` (valid xGov status, not revoked);
- `h ∈ [Bc; Bf)`;
- `a` has proposed at least one block in `[Bi; Bf)`.

The _voting power_ assigned to each xGov `(a, h)` is equal to the number of blocks
...
> such that `h ∈ [Bc; Bf)`, with no need to reacknowledge the xGov Registry for each
> period.
...
1. **Eligibility**: For all `(a, v)` in `C`, `a` is an _eligible_ xGov in `[Bc; Bf)` at `Bf`;
...
1. For each Block Proposer address (`a`) in `P`, assign a voting power (`v`) equal
to the number of blocks proposed in the governance period `[Bi; Bf)`.

1. (Optional) If the _revoked_ xGov stauts are not persisted on the xGov Registry,
replay state transitions to reconstruct the xGov eligibility at `Bf`.

1. Collect all the _eligible_ xGovs in the governance period `[Bc; Bf)` to build
the _eligible xGovs_ set `E(Bi, Bf)`.

1. Filter `P ∩ E` to obtain the _xGov Committee_ `C`.
