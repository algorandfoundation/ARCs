...
Upon each transfer the the Royalty Enforcer MUST assert that `AuthAddr == Sender` for the Seller (`from` parameter), in other words the Seller account has not been Rekeyed.

> The rational for this check is ensuring that `Rekeying` is not exploited to transfer the `royalty_asset` ownership circumventing the Royalty Policy. Without this check the two parties involved in the trade could create a Group Transactions in which: 1. Seller rekeys the `seller_account` to the Buyer `buyer_account`, 2. Buyer sends payment to `other_seller_account`. Exceptions can be handled with `Move` method.
