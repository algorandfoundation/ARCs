...
author: Ben Guidarelli (@barnjamin), Steve Ferrigno (@nullun) 
discussions-to: <URL>
status: Draft
...
A Royalty Share consists of an `address` that should receive a Royalty payment and a `share` representing some share of the total payent amount. The Royalty Share MUST be specified in terms of [basis points](https://en.wikipedia.org/wiki/Basis_point) of the payment amount.

A Royalty Policy is specified as a list of a list of addresses and associated Royalty weighting. Additionaly, if any assets besides Algos are acceptable, 
...
The `royalty_shares` are encoded as an array of `tuple(address, uint64)` that represents the Royalty Recipient and Royalty Share in basis points. The address of a Royalty Recipient SHOULD NOT be repeated, only the last occurence of the address will be used for their Royalty Share. 
...
Grant (TODO:naming?)
    ...
    grant(royalty_asset: uint64, grantee: address)

Grants an asset as royalty free by removing the `frozen` setting so transfers may be made normally.


## Rationale
The motivation behind defining a royalty enforcement specification is the need to guarentee a portion of a payment is received by select addresses on sale of an asset. Current royalty implementations are either platform specific or are only adhered to when an honest seller complies with it, allowing for the exchange of an asset without necessarily paying the royalties.

The use of a smart contract as a clawback address is a guarenteed way to know an asset transfer is only ever made when certain conditions are met, or made in conjunction with additional transactions. To make things easier on the purchaser, the smart contract can also handle the calculations required in dividing up and dispensing the payments to the respective parties by utilising inner transactions.
