...
Whilst it's not part of the ARC, an ARC-55-compliant contract **MAY** be destroyed once it is no longer needed. The process **SHOULD** be performed by the application creator, reclaiming any outstanding Algo funds. However it's important to note that destroying the application does not render the multisignature account inaccessible, as a new deployment with the same multisignature metadata can be configured and used.

Below is a typical expected lifecycle:

 * Admin deploys an ARC-55 compliant smart contract.
