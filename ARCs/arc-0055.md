...
Whilst it's not part of the ARC, an [ARC-55](./arc-0055.md)-compliant contract **MAY** be destroyed once it is no longer needed. The process **SHOULD** be performed by the application creator, reclaiming any outstanding Algo funds. However it's important to note that destroying the application does not render the multisignature account inaccessible, as a new deployment with the same multisignature metadata can be configured and used.
...
|--------|-------------|---------|----------------------------------------------------------|
| Global | "Threshold" | uint64  | The multisig signature threshold                         |
...
| Box    | n+i         | byte[]  | The ith transaction data for the nth transaction group   |
...
The associated read-only method is `arc55_transaction(uint64,uint8)byte[]`, which will return the transaction for a given transaction group nonce and transaction index. Note: To retrieve data larger than 1024 bytes, simulate must be called with `AllowMoreLogging` set to true.
...
The associated read-only method is `arc55_signatures(uint64,address)byte[64][]`, which will return the signatures for a given transaction group nonce and signer address.
