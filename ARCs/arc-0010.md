...
* `enable`: as specified in [ARC-6](./arc-0006.md#interface-enablefunction).
* `signAndPostTxns`: as specified in [ARC-8](./arc-0008.md#interface-signandposttxnsfunction).
* `getAlgodv2Client` and `getIndexerClient`: as specified in [ARC-9](./arc-0009.md#specification).
* `signTxns`: as specified in [ARC-5](./arc-0005.md#interface-signtxnsfunction) / [ARC-1](./arc-0001.md#interface-signtxnsfunction).
* `postTxns`: as specified in [ARC-7](./arc-0007.md#interface-posttxnsfunction).
...
* If `signAndPostTxns`, `getAlgodv2Client`, `getIndexerClient`, `signTxns`, or `postTxns` are called before `enable` (or `enableAccounts`), they **SHOULD** throw an error object with property `code=4202`. (See Error Standards in [ARC-1](arc-0001.md#error-standards)).
...
To handle these cases, we adopt and extend the [ARC-1](./arc-0001.md#interface-wallettransaction) format for `WalletTransaction`s that do not need to be signed:
...
* `stxn` is a `SignedTxnStr`, as specified in [ARC-7](./arc-0007.md#string-specification-signedtxnstr).
* For production wallets, `stxn` **MUST** be checked to match `txn`, as specified in [ARC-1](./arc-0001.md#semantic-and-security-requirements).
...
Where `makeTxns` is comparable to what is seen in [ARC-1](./arc-0001.md#reference-implementation)'s sample code.
