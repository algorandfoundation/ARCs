...
* **MAY** contain a file with the basename `application` followed by the extension of the high-level language the application is written in 
* **MAY** contain the files `approval.teal` and `clear.teal`, that are the compiled versions of approval and clear program in TEAL. 
    * Note that `approval.teal` will not be able to contain the application information as this would create circularity. If `approval.teal` is provided, it is assumed that the *actual* `approval.teal` that is deployed has the proper `bytecblock` as defined below prepended to it.
...
For public networks (e.g., MainNet, TestNet, BetaNet), block explorers and wallets (that support ARC-23) **SHOULD** try to recover application information files from IPFS, and if not possible, **SHOULD** allow developers to upload a CAR file.
...
> Example: For the example stored in `/asset/arc-0023/application_information`, the CID is `bafybeibm6rgofb2jn6iibyo67vchj6r3s3v3xkho7xkydlhy7inalysxxa`, which can be obtained with the command:
>
> ```
...
> Example: for the above CID `bafybeibm6rgofb2jn6iibyo67vchj6r3s3v3xkho7xkydlhy7inalysxxa`, the binary CID corresponds to the following hexadecimal value: 
>
> ```
> 0x017012202cf44ce287496f9080e1defd4474fa3b96ebbba8eefdd581acf8fa1a05e257b8
> ```
>
> and hence the encoded information byte string has the following hexadecimal value:
>
> ```
> 0x6172633233017012202cf44ce287496f9080e1defd4474fa3b96ebbba8eefdd581acf8fa1a05e257b8
> ```
...
>
> ```
> bytecblock 0x6172633233017012202cf44ce287496f9080e1defd4474fa3b96ebbba8eefdd581acf8fa1a05e257b8
> ```
>
...
However, if `approval.teal` is provided as an application information file, the `bytecblock` **SHOULD** be the last opcode of the TEAL program.
...
>
> ```
> bytecblock 0x6172633233017012202cf44ce287496f9080e1defd4474fa3b96ebbba8eefdd581acf8fa1a05e257b8 0x42
> ```
...
- example of the contract metadata that follow [ARC-4](arc-0004.md) [contract.json](../assets/arc-0023/Folder/contract.json).
