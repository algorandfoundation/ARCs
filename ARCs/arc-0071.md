...
- In current settings the SBT can only be closed out to ASA creator address.
- In current settings Manager address will be able to revoke the SBT ASA by setting the Reserve address to `ZeroAddress`.
- This ARC assumes a basic trust to be existing between Issuer and Receiver and that Issuer is trusted by Receiver to play fair.
