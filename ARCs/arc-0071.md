...
- There are 2 SBT ASA states,  **Issued** and **Revoked**. 
- **Issued** and **Revoked** SBTs reside in the holder's wallet after claim , forever!
...
The Issued state is the default state of the ASA. The Revoked state is the state where the SBT ASA is revoked by issuer after issuance and therefore no longer valid for any usecase except for provenance and historical data reference.
...
- The Manager parameter **MUST** be set to `ZeroAddress` in revoked SBT.
- The Reserve parameter **MAY** be set to either ARC19 metadata or SBT Issuer vault address.
