...
category: Interface
created: 2021-08-09
adoption-summary: adoption/arc-0010.yaml
---
## Abstract
...
* `enable`: as specified in [ARC-6](/arc-standards/arc-0006#interface-enablefunction).
* `signAndPostTxns`: as specified in [ARC-8](/arc-standards/arc-0008#interface-signandposttxnsfunction).
* `getAlgodv2Client` and `getIndexerClient`: as specified in [ARC-9](/arc-standards/arc-0009#specification).
* `signTxns`: as specified in [ARC-5](/arc-standards/arc-0005#interface-signtxnsfunction) / [ARC-1](/arc-standards/arc-0001#interface-signtxnsfunction).
* `postTxns`: as specified in [ARC-7](/arc-standards/arc-0007#interface-posttxnsfunction).
...
* If `signAndPostTxns`, `getAlgodv2Client`, `getIndexerClient`, `signTxns`, or `postTxns` are called before `enable` (or `enableAccounts`), they **SHOULD** throw an error object with property `code=4202`. (See Error Standards in [ARC-1](/arc-standards/arc-0001#error-standards)).
...
To handle these cases, we adopt and extend the [ARC-1](/arc-standards/arc-0001#interface-wallettransaction) format for `WalletTransaction`s that do not need to be signed:
...
* `stxn` is a `SignedTxnStr`, as specified in [ARC-7](/arc-standards/arc-0007#string-specification-signedtxnstr).
* For production wallets, `stxn` **MUST** be checked to match `txn`, as specified in [ARC-1](/arc-standards/arc-0001#semantic-and-security-requirements).

`signAndPostTxns` **MAY** reject when none of the transactions need to be signed by the user.

## Rationale

In order for a wallet to be usable by a DApp, it must support features for account discovery, signing and posting transactions, and querying the network.
...
Where `makeTxns` is comparable to what is seen in [ARC-1](/arc-standards/arc-0001#reference-implementation)'s sample code.
