...
> The xGov status of an address `a` is lost if pair `(a, h)` is removed from the
> xGov Registry, either by the address itself or by the xGov Registry rules.
