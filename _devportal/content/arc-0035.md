...
sub-category: Wallet
created: 2023-01-03
adoption-summary: adoption/arc-0035.yaml
---
## Abstract
...
1. The wallet derives a 32-byte key: `key = HMAC-SHA256(key="Algorand export 1.0", input=S)`
...
1. The wallet encrypts the input JSON using `crypto_secretbox_easy` from libsodium (<https://libsodium.gitbook.io/doc/secret-key_cryptography/secretbox>)
1. The wallet outputs the following output JSON:
...
1. S is encoded as a 12-word mnemonic (according to BIP-39) and displayed to the user.
