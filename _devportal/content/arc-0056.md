...
adoption-summary: adoption/arc-0056.yaml
requires:
  - 4
  - 22
  - 28
...
The types defined in [ARC-4](/arc-standards/arc-0004) may not fully describe the best way to use the ABI values as intended by the contract developers. These type interfaces are intended to supplement ABI types so clients can interact with the contract as intended.
...
Describes all the state that MAY exist in the app and how one should decode values. The schema provides the required schema when creating the app.

### Named Structs

It is common for high-level languages to support named structs, which give names to the indexes of elements in an ABI tuple. The same structs should be usable on the client-side just as they are used in the contract.

### Action

This is one of the biggest deviations from ARC-32, but provides a much simpler interface to describe and understand what any given method can do.
