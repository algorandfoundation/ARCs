...
category: Interface
created: 2021-08-09
adoption-summary: adoption/arc-0007.yaml
---
## Abstract
...
Defined as in [ARC-1](/arc-standards/arc-0001#interface-signedtxnstr):
...
`PostTxnsError` follows the same rules as `SignTxnsError` from [ARC-1](/arc-standards/arc-0001#error-interface-signtxnserror) and uses the same status codes as well as the following status codes:
...
* If unsuccessful, `postTxns` **MUST** reject the promise with an error `err` of type `PostTxnsError` such that:
  * `err.code=4400` if there was a failure sending the transactions or a code as specified in [ARC-1](/arc-standards/arc-0001#error-standards) if the user or function disallowed posting the transactions.
