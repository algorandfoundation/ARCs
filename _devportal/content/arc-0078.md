...
- type: fixed to "keyreg". Used to disambiguate the transaction type from the base [ARC-26](/standards/arcs/arc-0026) standard and other possible extensions. Required.
...
- xnote: As in [ARC-26](/standards/arcs/arc-0026). A URL-encoded notes field value that must not be modifiable by the user when displayed to users. Optional.

- note: As in [ARC-26](/standards/arcs/arc-0026). A URL-encoded default notes field value that the the user interface may optionally make editable by the user. Optional.
