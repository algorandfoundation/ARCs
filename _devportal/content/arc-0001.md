...
1. Otherwise, the wallet **MUST** sign the transaction `txns[i].txn` and `ret[i]` **MUST** be set to the corresponding `SignedTxnStr`.
...
1. (**OPTIONAL**) `txns` is a concatenation of `txns` arrays of transactions of type 1:
