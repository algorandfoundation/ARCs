...
The `arc62_get_circulating_supply` **MUST** be a _read-only_ ([ARC-22](/standards/arcs/arc-0022))
method (getter).
...
If the ASA also conforms to any ARC that supports additional `properties` ([ARC-3](/standards/arcs/arc-0003),
[ARC-19](/standards/arcs/arc-0019), etc.) as metadata declared in the ASA URL field,
...
App ID as [ARC-2](/standards/arcs/arc-0002) `AssetConfig` transaction note field, as follows:
...
> global storage, box storage, etc. See [Example 2](/standards/arcs/arc-0062#example-2) for
> reference.
