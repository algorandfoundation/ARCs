...
category: Interface
created: 2021-08-09
adoption-summary: adoption/arc-0006.yaml
---
## Abstract
...
This document requires nothing else, but further semantic meaning is prescribed to these functions in [ARC-10](/arc-standards/arc-0010#semantic-requirements) which builds off of this one and a few others.
...
Defined as in [ARC-1](/arc-standards/arc-0001#interface-algorandaddress):

> An Algorand address is represented by a 58-character base32 string. It includes includes the checksum.

#### Error Standards

`EnableError` follows the same rules as `SignTxnsError` from [ARC-1](/arc-standards/arc-0001#error-interface-signtxnserror) and uses the same status error codes.
...
The `enable` function **MAY** remember the choices of the user made by a specific caller and use them every time the same caller calls the function.
